...
{
	if(stream[i+1] != 'x' && stream[i+1] != 'X' && isdigit(stream[i+1]) == 0)
	{
		returned[j] = stream[i];
		i++;
		j++; 
...
int isHex(char* returned, char* stream, int i, int j, TokenizerT * tk)
{
 ...
 /*
  * isFloat: checks if the next token in TKGetNextToken is a Float.
  */
...
int isFloat(char* returned, char* stream, int i, int j, TokenizerT * tk)
{
	...
	// Aman
	int length = i + 2;
	if(isdigit(stream[length]) == 0)
	{
		length = (-1) * length;
		return length;
	}
	while(isdigit(stream[length]))
		length++;
	if(stream[length] == 'e' || stream[length] == 'E')
	{
		length++;
...
char *TKGetNextToken( TokenizerT * tk ) {
    ...
    if (isdigit(stream[i])) 
	{
        ...
        if(stream[i] == '0') 
		{
			...
			{
				int length = isFloat(returned, stream, i, j, tk);
				if(length > 0)
				{
				...
				else
				{
					...
					//tk->pos = i;
					int x;
					for(x = 0; x<2; x++) // to copy all previous digits and proceed 0.
					{
						returned[j] = stream[i];
						i++;
						j++;
    ...
    return "something is wrong, not supposed to reach this state";
}
